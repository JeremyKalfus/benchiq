...
- active experiment plan note:
  [docs/design/generalization_optimization_plan.md](/Users/jeremykalfus/CodingProjects/BenchIQ/docs/design/generalization_optimization_plan.md)
...
- The next implementation work is:
  ...
  - T21 GAM vs Elastic Net vs XGBoost reconstruction experiments
  - T22 less-stochastic preselection alternative and comparison
  - T23 factor-analysis demotion from the core product path
  - T24 paper-ready experiment and calibration/deployment reporting bundle
- T19 is now complete: reusable `calibrate` / `predict` APIs and CLI entrypoints exist, calibration
  bundles are exported, and fit-once / predict-later integration coverage passes.
- T20 is now complete as an optional parity harness: simulated-data comparison helpers, tests, a
  design note, and saved reports exist; the local environment currently skips the actual R baseline
  because `mirt` is not installed.
- T21 is now complete on the compact experiment fixture: GAM, Elastic Net, and XGBoost comparison
  code, tests, scripts, tables, plots, and notes are saved under `reports/experiments/`.
- T22 is now complete on the compact experiment fixture: the deterministic
  `deterministic_info` stage-04 alternative exists, is tested, and has saved comparison outputs
  under `reports/selection_comparison/`.
- T23 is now complete at the product-story layer: calibration / deployment / reconstruction lead
  the README and CLI docs, while redundancy is documented as secondary analysis.
- T24 is now complete for the compact saved bundle: calibration/deployment, reconstruction-head,
...
Scope:
- add a first-class calibration artifact bundle and reusable fit API
- add a first-class deployment/predict API that loads a fitted bundle and scores new reduced responses
- add CLI entrypoints that make the split explicit without breaking `benchiq run`
- preserve artifact-first manifests and explicit failure behavior

Acceptance criteria:
- fit once / predict later works in integration tests
- deployment never silently retrains
- missing fitted artifacts fail clearly
- docs explain the distinction between calibration and deployment

Blockers:
- fitted-bundle schema drift across stages
- deployment-time feature construction not matching calibration-time definitions
...
Scope:
- add simulated-data parity tests against an R baseline, preferably `mirt`
- keep R optional and test-only
- save parity notes and reviewer-facing tables under a stable reports path

Acceptance criteria:
- parity tests skip clearly if R or required packages are missing
- parameter, theta-ordering, and probability-level comparisons are tolerance-based and honest
- a short design note documents alignment and non-identifiability caveats

Blockers:
- unavailable R packages in the local environment
- scale/sign alignment complexity if parameterizations differ materially

### T21 reconstruction head experiments

Depends on: T19

Scope:
- keep GAM as the product baseline
- add Elastic Net and XGBoost experiment heads
- add a reusable experiment harness and saved comparison artifacts

Acceptance criteria:
- same data, same splits, same reduced subsets across methods
- saved outputs include RMSE, MAE, Pearson, Spearman, runtime, and stability
- a concise note states what won and why

Blockers:
- dependency/setup friction for the tree baseline
- experiment runtime if the comparison matrix grows too large
...
Scope:
- keep random CV preselection
- add at least one deterministic or less-stochastic alternative
- compare quality, runtime, seed variance, and selected-item stability

Acceptance criteria:
- artifact outputs exist for both baseline and alternative methods
- the recommendation is backed by saved comparison outputs
- the baseline method remains available

Blockers:
- alternative selection quality may not beat the random baseline
- stability metrics may need careful interpretation for small fixtures
...
Scope:
- make factor analysis secondary in docs, CLI help, and core messaging
- keep redundancy/compressibility available
- move factor analysis behind an optional analysis path if practical

Acceptance criteria:
- README and CLI docs lead with distillation, calibration, deployment, and reconstruction
- factor analysis is no longer treated as a headline core metric

Blockers:
- preserving backward compatibility for the existing `run` path and reports
...
Scope:
- standardize report locations under `reports/`
- save machine-readable tables, markdown summaries, and plots for each experiment family
- include a calibration/deployment walkthrough and an honest "what improved / what did not" note

Acceptance criteria:
- repo-stable output locations exist for all requested comparisons
- plots and markdown summaries are present alongside machine-readable tables
- every claim in the updated docs is traceable to a saved artifact

Blockers:
- unfinished upstream comparisons leave gaps in the report bundle
