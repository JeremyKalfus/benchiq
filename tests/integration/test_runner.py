...
def test_runner_executes_full_pipeline_and_partial_rerun_deterministically(tmp_path) -> None:
    ...
    assert "10_redundancy" not in metrics_report["stage_records"]
    assert not (run_result.run_root / "artifacts" / "10_redundancy").exists()
...
def test_run_wrapper_includes_stage_10_only_when_requested(tmp_path) -> None:
    responses_path = _write_synthetic_bundle(tmp_path)
    ...
    run_result = benchiq.run(
        ...
        _build_config(),
        out_dir=tmp_path / "out",
        run_id="runner-toy-redundancy",
        include_redundancy=True,
        stage_options=_build_stage_options(),
    )

    assert run_result.executed_stages[-1] == "10_redundancy"
    assert run_result.summary()["metrics"]["theta_correlation_summary"]
    assert run_result.summary()["metrics"]["factor_analysis"]
    assert (run_result.run_root / "artifacts" / "10_redundancy").exists()
...
def test_runner_serializes_missing_joint_metrics_when_joint_path_is_skipped(tmp_path) -> None:
    ...
    assert metrics_report["executed_stages"][-1] == "09_reconstruct"
    assert "theta_correlation_summary" not in metrics_report["metrics"]
    assert "factor_analysis" not in metrics_report["metrics"]
...
def _build_stage_options() -> dict[str, object]:
    return {
        "04_subsample": {
            "k_preselect": 4,
            "n_iter": 4,
